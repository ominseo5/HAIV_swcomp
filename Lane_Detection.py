...
cv2.createTrackbar("ignore", "control",84, 140, env.nothing)
...
while True:
    ...
    # @ 장애물 회피 알고리즘
    # get = int(serial_comm('r0'))
    # if get < 1000:
    #     ws = 0
    #     cv2.setTrackbarPos("throtel", "control", 0)

    # @ 신호등 알고리즘
    # traffic, width = env.detect_traffic_light(frame2,model_traffic,device)
    # print(f"label:{traffic}, width:{width}")
    # if width > 220 and traffic == 'red':
    #     serial_one('x')
    #     serial_one('c')
    #     condition = 1
    # if traffic == 'green' and condition == 1:
    #     condition = 0
